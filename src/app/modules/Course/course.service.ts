...
const getPaginatedAndFilterCoursesFromDB = async (query:Record<string,any>)=>{
    ...
    if (sortBy && !allowedSortFields.includes(sortBy as string)) {
      throw new Error(`Invalid sortBy field. Allowed fields are: ${allowedSortFields.join(', ')}`);
    }
    ...
    const result = await Course.find().sort(sortOptions).skip(skip).limit(parseInt(limit as string));
